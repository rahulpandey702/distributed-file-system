  ...
  partitioned (false), dopartition(false), break1(false), break2(false)
{
  ...
  rsmrpc->reg(rsm_protocol::invokereplica, this, &rsm::invoke);
  ...
  while (1) {
    while (!cfg->ismember(cfg->myaddr(), vid_commit)) {
      if (join(primary)) {
        ...
        //printf("Recovery waits here till the node becomes a part of the latest commit group\n");
        Sleep_ms(1); // XXX make another node in cfg primary?
    ...
    tprintf("recovery: sync done %d\n", r);
    tprintf("recovery current view %d, commited view %d\n", vid_insync, vid_commit);
    ...
    if (r) { 
      ...
      myvs.seqno = 1;
      inviewchange = false;
      tprintf("DONE VIEWCHANGE\n");
    }
    tprintf("recovery: go to sleep %d %d\n", insync, inviewchange);
    VERIFY(pthread_cond_wait(&recovery_cond, &rsm_mutex) == 0);
  }
}
...
rsm::sync_with_backups()
{
  ...
  while(!backups.empty() && vid_insync == vid_commit) {
    ...
    ts.tv_sec += 3;
    tprintf("sync_with_backups: Waiting for backups to be synchronized or for a view change. Backups empty? %d or vid still match? %d Backups left %d\n", backups.empty(), vid_insync == vid_commit, backups.size());
    int ret = pthread_cond_timedwait(&sync_cond, &rsm_mutex, &ts) ;
    for(unsigned int i = 0; i < backups.size(); i++) {
      tprintf("Backups left: %s\n", backups[i].c_str());
      break;
    }
    if(ret != ETIMEDOUT) {
      VERIFY(ret == 0);
  ...
  if(backups.empty()) {
    tprintf("sync_with_backups: synchornized with all backups\n");
  }
  if(vid_insync != vid_commit) {
    tprintf("sync_with_backups: view has changed aborting the synchronization\n");
    r = false;
  }
  VERIFY(pthread_cond_signal(&recovery_cond) == 0);
  insync = false;
  return r;
}
...
rsm::sync_with_primary()
{
  ...
  insync = true;
  bool r = false;
  while(!r && vid_insync == vid_commit) {
    r = statetransfer(m);
    if(!r) {
      tprintf("rsm::sync_with_primary statetransfer for replica %s returns failure, retrying statetransfer\n", m.c_str());
      Sleep_ms(1);
    }
    else {
      tprintf("rsm::sync_with_primary statetransfer for replica %s succeeds\n", m.c_str());
    }
  }
  if(vid_insync != vid_commit) {
    tprintf("rsm::sync_with_primary statetransfer abandoned as view changed\n");
  }
  else {
...
rsm::statetransfer(std::string m)
{
  ...
  rpcc *cl = h.safebind();
  if (cl) {
    ret = cl->call(rsm_protocol::transferreq, cfg->myaddr(), last_myvs, vid_insync, r, rpcc::to(500));
  }
  VERIFY((pthread_mutex_lock(&rsm_mutex))==0);
  if (cl == 0 || ret != rsm_protocol::OK) {
    tprintf("rsm::statetransfer: failed %s %d\n", m.c_str(), ret);
    return false;
  }
  if (stf && last_myvs != r.last) {
    tprintf("rsm::statetransfer: is unmarshalled\n");
    stf->unmarshal_state(r.state);
  }
  tprintf("rsm::statetransfer from %s success, before lastmyvs(%d,%d)\n", m.c_str(), last_myvs.vid, last_myvs.seqno);
  last_myvs = r.last;
  myvs = last_myvs;
  myvs.seqno ++;
  tprintf("rsm::statetransfer from %s success, after last myvs(%d,%d)\n", m.c_str(), last_myvs.vid, last_myvs.seqno);
  return true;
}
...
rsm::statetransferdone(std::string m) {
  ...
  if (cl) {
    int dummy;
    tprintf("call transferdonereq\n");
    ret = cl->call(rsm_protocol::transferdonereq, cfg->myaddr(), vid_insync, dummy, rpcc::to(500));
  }
  VERIFY(pthread_mutex_lock(&rsm_mutex)==0); 
  if(cl == 0 || ret != rsm_protocol::OK) {
    tprintf("rsm::statetransferdone for primary %s failed\n", m.c_str());
    res = false;
  }
  else {
    tprintf("rsm::statetransferdone informed primary %s about completion of synchronization\n", m.c_str());
    res = true;
  }
  return res;
}
...
rsm::commit_change_wo(unsigned vid) 
{
  ...
  vid_commit = vid;
  inviewchange = true;
  tprintf("BEGIN VIEWCHANGE\n");
  tprintf("commit_change_wo: in case of a primary node, notify itself that view has changed so that it can abort synchronization of older views\n"); 
  VERIFY(pthread_cond_signal(&sync_cond) == 0);
  set_primary(vid);
  //Backups for the primary are recorded here
  if(primary == cfg->myaddr()) {
    ...
    for(itr = mem.begin(); itr != mem.end(); itr++) {
      if(*itr != cfg->myaddr()) {
        printf("Pushing %s of view %d in backups\n", itr->c_str(), vid_commit);
        backups.push_back(*itr);
      }
    }
  }
  VERIFY(pthread_cond_signal(&recovery_cond) == 0);
...
rsm::client_invoke(int procno, std::string req, std::string &r)
{
  ...
  // You fill this in for Lab 7
  tprintf("client invoke %x req %s\n", procno, req.c_str());
  viewstamp vs ;
  std::string myaddr;
  {
    ScopedLock ml(&rsm_mutex);
    if(inviewchange == true) {
      tprintf("client invoke BUSY %x req %s\n", procno, req.c_str());
      ret = rsm_client_protocol::BUSY;
    }
    else if(primary != cfg->myaddr()) {
      tprintf("client invoke NOTPRIMARY %x req %s\n", procno, req.c_str());
  ...
  if(ret == rsm_client_protocol::OK) {
    {
      ...
      for(itr = m.begin(); itr != m.end(); itr++) {
        std::string replica = *itr;
        if(replica == myaddr) {
          //tprintf("rsm:: invoke skipped for replica since it's primary %s %s\n", myaddr.c_str(), replica.c_str());
          continue;
        }
        handle h(replica);
        tprintf("rsm:: client_invoke %s calling invoke for replica %s . My last_myvs(%d,%d)\n", cfg->myaddr().c_str(), replica.c_str(), last_myvs.vid, last_myvs.seqno);
        rpcc *cl = h.safebind();
        if (cl) {
          int dummy;
          tprintf("rsm:: client_invoke calling invoke at replica for %s\n", replica.c_str());
          ret1 = cl->call(rsm_protocol::invokereplica, procno, vs, req, dummy, rpcc::to(1000));
        }
        if (cl == 0 || ret1 != rsm_client_protocol::OK) {
          tprintf("rsm:: client_invoke invoke call at replica %s failed ret: %d\n", replica.c_str(), ret1);
          ret = rsm_client_protocol::BUSY;
          break;
        }
        else {
...
rsm::invoke(int proc, viewstamp vs, std::string req, int &dummy)
{
  ...
  // You fill this in for Lab 7
  {
    tprintf("invoke for %x (vs:vid %d :seqno %d) (myvs: vid %d seqno %d)\n", proc, vs.vid, vs.seqno, myvs.vid, myvs.seqno);
    ScopedLock ml(&rsm_mutex);
    if(myvs != vs) {
      tprintf("invoke %x wrong view sequence (vs:vid %d :seqno %d) (myvs: vid %d seqno %d)\n", proc, vs.vid, vs.seqno, myvs.vid, myvs.seqno);
      ret = rsm_protocol::ERR;
    }
    else if(inviewchange){
      tprintf("invoke BUSY %x\n", proc);
      ret = rsm_protocol::BUSY;
    }
    else if(primary == cfg->myaddr()){
      tprintf("invoke %x received at primary node\n", proc);
      ret = rsm_protocol::ERR;
    }
  }
  if(ret == rsm_protocol::OK) {
    std::string r;
    tprintf("invoke %x executed\n", proc);
    execute(proc, req, r);
    {
      ScopedLock ml(&rsm_mutex);
      last_myvs = myvs;
    ...
    rsm_protocol::transferres &r)
{
  ...
  // Code will be provided in Lab 7
  tprintf("transferreq from %s (%d,%d) vs (%d,%d) vid %d vid_insync %d\n", src.c_str(), last.vid, last.seqno, last_myvs.vid, last_myvs.seqno, vid, vid_insync);
  if (!insync || vid != vid_insync) {
    tprintf("transferreq from %s (%d,%d) rejected %d (vid %d vid_insync %d)\n", src.c_str(), last.vid, last.seqno, insync, vid, vid_insync);
    ret = rsm_protocol::BUSY;
  }
  else {
    if (stf && last != last_myvs) { 
      tprintf("transferreq from %s (%d,%d) is marshalled\n", src.c_str(), last.vid, last.seqno);
...
rsm::transferdonereq(std::string m, unsigned vid, int &)
{
  ...
  if(!insync) {
    ret = rsm_protocol::BUSY;
    tprintf("transferdonereq: for replica:%s vid:%d failed as primary is not currently insync. Returning BUSY\n", m.c_str(), vid);
  }
  else if(vid != vid_insync){
    ret = rsm_protocol::BUSY;
    tprintf("transferdonereq: for replica:%s vid:%d failed as primary is not currently as there is view mismatch:%d. Returning BUSY\n", m.c_str(), vid, vid_insync);
  }
  else {
    for(unsigned int i = 0; i < backups.size(); i++) {
      if(backups[i] == m) {
        backups.erase(backups.begin()+i);
        tprintf("transferdonereq: for replica:%s vid:%d done and erased from unsynchronized backups\n", m.c_str(), vid);
        break;
      }
    }
    if(backups.empty()) {
      tprintf("transferdonereq: primary: %s unsynchronized backup list is now empty\n", cfg->myaddr().c_str());
      VERIFY(pthread_cond_signal(&sync_cond) == 0);
    }
  }
  return ret;
...
rsm::set_primary(unsigned vid)
{
  ...
  if (isamember(primary,c)) {
    ...
    FILE *fp = fopen("primary.log", "w");
    if(fp) {
      fprintf(fp,"%s",primary.c_str());
      fclose(fp);
    }
    return;
  }
...
{
  ScopedLock ml(&rsm_mutex);
  printf("amiprimary: cfg: %s primary: %s inviewchange: %d\n",cfg->myaddr().c_str(), primary.c_str(), inviewchange);
